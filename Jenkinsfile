...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Deploye Code') {
			...
			if (isUnix()) {
				rmsg = sh returnStdout: true, script: "sfdx force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --instanceurl ${SFDC_HOST} --username ${HUB_ORG} --jwtkeyfile ${jwt_key_file}  & sfdx force:source:deploy -u ${HUB_ORG} -p "force-app" -w"
			}else{
